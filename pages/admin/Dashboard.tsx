...
const AdminDashboard: React.FC = () => {
  ...
  const { settings, t, currentLanguage, formatCurrency, formatDuration, formatTime } = useSettings();
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-4 sm:grid-cols-6 gap-1.5 mb-2.5">
                {availableSlots.lunch.length > 0 ? availableSlots.lunch.map(time => (
                  <button key={time} onClick={() => setSelectedTime(time)} className={`py-1.5 rounded text-xs font-medium transition-colors border ${selectedTime === time ? 'bg-gold-500 border-gold-500 text-dark-900' : 'bg-dark-900 border-dark-700 text-neutral-400 hover:border-neutral-500'}`}>{formatTime(time)}</button>
            ...
            {selectedTime && (
              <div className="bg-dark-900/80 border border-dark-700 rounded-xl p-3 mb-5 flex flex-col items-center">
                ...
                <div className="flex items-center gap-6">
                  <div className="text-center"><div className="text-[10px] text-neutral-500 mb-0.5">{t('booking.starts')}</div><div className="text-xl font-bold text-gold-500">{formatTime(selectedTime)}</div></div>
                  <ArrowRight className="text-neutral-600 mt-3" size={16} />
                  <div className="text-center"><div className="text-[10px] text-neutral-500 mb-0.5">{t('booking.ends')}</div><div className="text-xl font-bold text-gold-500">{formatTime(calculateEndTime(selectedTime, duration))}</div></div>
                </div>
              </div>
            )}
