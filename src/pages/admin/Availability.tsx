...
const AdminAvailability: React.FC = () => {
  ...
  const { settings, t, currentLanguage } = useSettings();
  ...
  const timeSlots = useMemo(() => {
    ...
    while (currentM < closeM) {
      ...
      slots.push({ label: timeStr, start: currentM, isBreak: false });
      currentM += 30;
    }
    return slots;
  }, [selectedDate, settings.booking_schedule, t]);
                 ...
                 <div><label className="block text-xs font-medium text-neutral-400 mb-1.5 uppercase tracking-wider">{t('booking.select_time')}</label>
                    <div className="grid grid-cols-4 gap-2 mb-2 max-h-40 overflow-y-auto">
                      {editOptions.lunch.length > 0 ? editOptions.lunch.map((time: string) => (
                        <button key={time} onClick={() => setEditSelectedTime(time)} className={`py-1.5 rounded-md text-xs font-medium transition-colors border ${editSelectedTime === time ? 'bg-gold-500 border-gold-500 text-dark-900' : 'bg-dark-900 border-dark-700 text-neutral-400 hover:border-neutral-500'}`}>{time}</button>
