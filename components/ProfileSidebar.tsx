...
export const ProfileSidebar: React.FC<ProfileSidebarProps> = ({ isOpen, onClose }) => {
  ...
  const getUserName = () => {
    ...
    return user?.email || t('auth.guest_user');
  };

  const userName = user ? getUserName() : (isGuest ? t('auth.guest_user') : t('auth.welcome_guest'));
  const userPhone = profile?.phone || "";
  const userEmail = profile?.email || user?.email || t('auth.please_sign_in');
  ...
  const menuItems = [
    ...
    { icon: Globe, label: t('navigation.language'), sub: currentLanguage.code.toUpperCase(), path: "#" },
